class Series
  ...
  def slices(length)
    if length > number.length
      ...
      self.subarray(length)
    end
  end

  def subarray(length_of_series)
    num = number.split('').map {|num| num.to_i}
    array = []
    length = number.length
    (0..length-1).each do |start|
      (start..length -1).each do |stop|
        array << num[start..stop]
      end
    end
    new_array = array.uniq.select {|subarray| subarray.size == length_of_series}
  end
end
