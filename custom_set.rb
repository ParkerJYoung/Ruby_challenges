class CustomSet
  ...
  def eql?(other_set)
    @set.all? {|num| other_set.contains?(num)} && @set.size == other_set.set.size
  end
  ...
  def difference(other_set)
    CustomSet.new(@set.select {|num| other_set.contains?(num) == false})
  end

  def union(other_set)
    array = []
    @set.each {|num| array << num}
    other_set.set.each {|num| array << num}
    CustomSet.new(array)
  end
end
